...
class QUANTLHModel:
    """
    constructor
    @param n number of workplaces
    @param m number of residential zones
    """
    ...
    @param Sij trips matrix containing the flow numbers between employment zones (i) and residential zones (j)
    ...
    """
    def runTwoModes(self):
        # run model
        # i = employment zone
        # j = residential zone
        # Ei = number of jobs per zone
        # Aj = attractor of HH (floorspace)
        ...
        while not converged:
            ...
            print(
                "i= {0:d} beta pu={1:.6f} beta pr={2:.6f} cbar pred pu={3:.1f} ({4:.1f}) cbar pred pr={5:.1f} ({6:.1f})"
